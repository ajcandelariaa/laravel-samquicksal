...
<div class="pac-card" id="pac-card">
    <div>
        <form action="/restaurant/manage-restaurant/about/restaurant-information/updateLocation" id="locationForm" method="POST">
            @csrf
            <div class="grid grid-cols-2 items-center bg-blue-500 py-2">
                <div id="title">Please enter the address to locate your restaurant</div>
                <div class="text-right mr-5">
                    <button type="submit" id="btnLocSubmit" class="text-sm bg-submitButton rounded-lg py-1 px-7  hover:bg-btnHoverColor shadow-adminDownloadButton text-white">Submit</button>
                </div>
            </div>

            <p class="font-Montserrat ml-3 mt-5">Restaurant Address: <span class="font-semibold">{{ $account->rAddress.', '.$account->rBranch.', '.$account->rCity.', '.$account->rState.', '.$account->rCountry }}</span></p>
