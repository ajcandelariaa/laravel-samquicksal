...
<div class="container mx-auto">
    <div class="w-11/12 mx-auto mt-10 font-Montserrat bg-adminViewAccountHeaderColor2 pb-2">
        <div class="bg-manageRestaurantSidebarColorActive">
            <div class="uppercase font-bold text-white text-center text-xl py-3">Completed Customer Queue</div>
        </div>
                ...
                @for ($i=0; $i<sizeOf($storeCustomer); $i++)
                    @if ($count % 2 == 0)
                        <div class="grid grid-cols-11 py-2 text-center bg-manageFoodItemHeaderBgColor">
                            ...
                            <div class="col-span-1">
                                <a href="/restaurant/transaction-history/completed/queue/{{ $storeCustomer[$i]['bookId'] }}">
    ...
    @if ($storeCustomer == null)
        <div class="mx-auto w-11/12 mt-1 grid grid-cols-1 items-center bg-manageFoodItemHeaderBgColor h-14 shadow-xl rounded-lg">
            <div class="text-center text-multiStepBoxColor uppercase">There are no queue completed customer as of now</div>
        </div>
    @endif
</div>
@endsection